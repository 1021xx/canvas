...
class Canvas extends CanvasOption {
  ...
  createParticles(x, y, colorDeg) {
    ...
    for (let i = 0; i < PARTICLE_NUM; i++) {
      const r = randomNumBetween(1, 25) * hyptenuse(innerWidth, innerHeight) * 0.0001
      const angel = Math.PI / 180 * randomNumBetween(0, 360)
      const vx = r * Math.cos(angel)
      const vy = r * Math.sin(angel)
  ...
  render() {
    ...
    const frame = () => {
      ...
      this.ctx.fillStyle = this.bgColor + '40' // '10' #00000010 alph값
      ...
      this.sparks.forEach((spark, index) => {
        ...
        if (spark.opacity < 0) this.sparks.slice(index, 1)
      })
